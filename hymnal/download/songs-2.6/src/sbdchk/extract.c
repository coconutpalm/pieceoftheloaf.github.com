...
/* We count line and column numbers as we scan the .sbd file so as to display them
 * in warnings and error messages reported to the user in phase 2.
 */
...
/* Read the next character from the file. If it's a newline, increment the line counter.
 * If it's the end of the file (or an error) take some corrective action.
 */
#define IFEOF(f,c) \
	if (((c)=ws_fgetc(f))==wc_newline) { ++line; col=0; } \
	else if (++col, c==wc_EOF)

/* Test if it's appropriate to append a hyphen to the word in progress at this point.
 * Hyphens shouldn't appear at the very beginning of a word, nor should they appear
 * immediately after other hyphens.
 */
#define hyphenok(p) (((p)>word) && (*((p)-1) != wc_hyphen) && (*((p)-1) != wc_asterisk))

/* Write the word we've just finished scanning to the "fout" file. Omit trailing
 * hyphens, if any.
 */
#define COMMITWORD \
	...
	*wp = wc_null; \
	...
	if (!writeword(fout,(c==wc_newline) ? (line-1) : line,wordstartcol,word)) return -2; \
	wp = word

/* If we've reached the end of the .log file, output the main body code and terminate. */
	...
	FILE *fout;
{
	 ...
	 * the hyphenation reports will only show up in the log file, not on the screen.
	 */
	int firstword = 1;
		...
		else
		{
			...
			for (;;)
			{
				...
				else if (c == wc_backslash)
				{
					...
					IFEOF(fin,c)
					{
						fprintf(stderr, "sbdchk: file ended immediately after escape character\n");
						return -2;
					}
					if (c == wc_lbrack)
					{
						/* It's a chord macro. Append a hyphen and skip to the terminating ]. */
						if (hyphenok(wp))
						{
							if (wp >= word+MAXWORDLEN)
							{
						...
						do
						{
							IFEOF(fin,c)
							{
								fprintf(stderr, "sbdchk: file ended before chord macro was completed\n");
								return -2;
							}
						}
						while (c != wc_rbrack);
					...
					{
						/* It's (probably) an accent macro. Skip it and continue the current word. */
						IFEOF(fin,c) break;
					}
					else
					{
				...
				{
					/* We've reached a word-ending character like a space or punctuation. Write
					 * the word to the output file if it contains any hyphens.
					 */
					if (wp > word)
					{
						COMMITWORD;
			...
			/* We've just finished a verse or chorus. If any word was in progress, it is now
			 * ended so write it to the output file if it has any hyphens in it. Then resume
			 * scanning for the next verse or chorus.
			 */
			if (wp > word)
			{
				COMMITWORD;

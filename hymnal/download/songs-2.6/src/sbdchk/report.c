	...
	FILE *fout;
{
	...
	for (;;)
	{
		...
		{
			fprintf(stderr, "sbdchk: Identifier \"" IDSTRING "\" not followed by valid line number and word.\n");
			return -2;
		}
		...
		/* The third line in the sequence will contain LaTeX's hyphenation of the word.
		 * The line will look like "[] <fontname> <hyphenation>" so skip the first two
		 * words to reach the <hyphenation> part, which is the only part we want.
		 */
		 ...
		 * the .sbd word has a hyphen or asterisk but the LaTeX word does not have a
		 * hyphen. Those are places where there's a chord at a place that LaTeX doesn't
		 * recognize as a hyphenation point.
		 */
		...
		if (*swp || *twp)
		{
			ws_fprintf2(fout, ws_lit("%u:%u: Questionable hyphenation \""), line, col);
		...
		/* Now find any hyphen in the .sbd word that falls with one of the sequences
		 * "ff", "ffl", "ffi", "fi", or "fl". These are places where a ligature chord
		 * macro should have been used but wasn't.
		 */

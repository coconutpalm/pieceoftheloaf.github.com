 ...
 *
 * The program is written in straight ansi C, since at present that is
 ...
 * it, or go to http://www.sourceforge.net to obtain a Windows self-installer
...
extern int genscriptureindex(FSTATE *fs, const char *outname, const char *biblename);
	...
	char *argv[];
{
	...
	int retval;
	const char *bible, *inname, *outname;
	int i;

	bible = inname = outname = NULL;
    for (i=1; i<argc; ++i)
	{
		...
		{
			printf("Syntax: %s [-b " BIBLEDEFAULT "] input.sxd [output.sbx]\n", argv[0]);
			printf("If omitted, 'output.sbx' defaults to the input filename but with\n"
				   "the file extension renamed to '.sbx'. To read or write to stdin\n"
			       "or stdout, use '-' in place of input.sxd or output.sbx.\n"
